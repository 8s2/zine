...
public class RemapSpriteSource implements SpriteSource {
    ...
    ).apply(instance, RemapSpriteSource::new));

    private List<TextureSet> textureSets;
    private List<Mapping> mappings;

    public RemapSpriteSource(List<TextureSet> textureSets, List<Mapping> mappings) {
    ...
    public void run(ResourceManager resourceManager, Output regions) {
        ...
        for(Mapping mapping : this.mappings) {
            TextureData mappingData = SpriteSourceUtil.open(resourceManager, mapping.texture);
            ...
            for(int i = 0; i < this.textureSets.size(); i++) {
                ...
                regions.add(texture, spriteOpener -> {
                    ...
                    for(int j = 0; j < pixelData.length; j++) {
                        ...
                        nativeImage.setPixel(x, y, sampler.sample(pixelData[j], u, v));
                    }
    ...
    public record Mapping(Identifier texture, Optional<String> prefix, Optional<String> suffix) {
        ...
        public Identifier getTextureName(String name) {
            ...
            return Identifier.fromNamespaceAndPath(this.texture.getNamespace(), name);
        }
    }
    ...
    public record Texture(Identifier texture, Optional<Offset> offset) {
        ...
        public static final Codec<Texture> VALUE_CODEC = Codec.either(Identifier.CODEC, CODEC).xmap(
                either -> either.map(RemapSpriteSource::texture, Function.identity()),
                texture -> texture.offset.isPresent() ? Either.right(texture) : Either.left(texture.texture)
        );

        public Sampler createSampler(ResourceManager resourceManager) {
            TextureData textureData = SpriteSourceUtil.open(resourceManager, this.texture);
    ...
    public record Offset(Identifier texture, int min, int max) {
        ...
        public Sampler offsetSampler(Sampler sampler, ResourceManager resourceManager) {
            ...
            return (uvw, u, v) -> {
                ...
                int offsetU = Mth.lerpInt((offset >> 16 & 0xFF) / 255f, this.min, this.max);
                int offsetV = Mth.lerpInt((offset >> 8 & 0xFF) / 255f, this.min, this.max);
                sU = Mth.clamp(sU + offsetU, 0, 255);
                sV = Mth.clamp(sV + offsetV, 0, 255);
